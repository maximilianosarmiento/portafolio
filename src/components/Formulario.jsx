...
const Formulario = () => {
  const { t } = useTranslation();
    ...
    //Funcion de envio de datos a email. (emailjs)-----------------------
    function enviarEmail(params){
        console.log(params);

        emailjs.send('service_ucyzy7n','template_zzcgbf8', params, 'dsWGoHBUsPv8yohyq').then(res=>{
            console.log(res);
        })
    }
    ...
    <>
        <Formik
            ...
            { ({ values, errors, touched, handleSubmit , handleChange, handleBlur}) => (
                <form onSubmit={ handleSubmit } className='  flex  flex-col w-10/12 p-2 lg:pr-4 self-center'>
                {console.log(errors)}
                ...
                <textarea className='shadow-xl mt-4 bg-grisClaro border-2 border-naranja resize-none h-32 rounded-lg' name='textArea' id='textArea'  onChange={handleChange} value={values.textArea}></textarea>
                
                {formularioEnviado && <p className='text-green-600 text-center py-1'>{t("home.saludoContacto")}</p>}

                <>
                <button type='submit' className='text-naranja font-kodchasan font-medium mt-4  hover:text-grisClaro border-naranja hover:border-1 hover:bg-gradient-to-r from-naranja to-orange-600  border-2 w-fit rounded-xl px-2 py-1 transition ease-in duration-200 group'><a href="" className='flex items-center gap-1' >{t("home.enviar")}<FaChevronCircleRight className='group-hover:transform group-hover:translate-x-2 group-hover:transition group-hover:duration-200 mr-1'></FaChevronCircleRight></a></button>
                </>
